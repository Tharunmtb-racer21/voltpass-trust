        ...
        <nav className="hidden items-center gap-1 md:flex">
          {user && (
            <>
              ...
              <NavLink to="/batteries" icon={<Battery className="h-4 w-4" />}>Batteries</NavLink>
              <NavLink to="/batteries/register" icon={<Plus className="h-4 w-4" />}>Register</NavLink>
        ...
        <div className="flex items-center gap-2">
          {user ? (
            ...
            <>
              ...
              <Button asChild size="sm" className="bg-gradient-to-r from-primary to-electric-glow text-primary-foreground">
...
function NavLink({ to, icon, children }: { to: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <Link
      ...
      activeProps={{ className: "flex items-center gap-2 rounded-md px-3 py-2 text-sm bg-accent text-foreground" }}
    >
      {icon}
      {children}
    </Link>
...
export function ScanQrButton() {
  return (
    <Button variant="outline" size="sm" asChild>
      <Link to="/scan"><ScanLine className="mr-2 h-4 w-4" />Scan QR</Link>
    </Button>
  );
}
