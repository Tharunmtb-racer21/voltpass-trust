...
function PublicPassport() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("batteries").select("*").eq("id", id).maybeSingle();
    ...
    <main className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex items-center gap-2 text-xs text-success">
          <ShieldCheck className="h-4 w-4" />Verified Battery Passport
        </div>
        <h1 className="mt-2 font-display text-3xl font-bold">{data.battery_code}</h1>
        <p className="text-muted-foreground">{data.manufacturer} · {data.model}</p>
        ...
        <Button onClick={() => generatePdfReport(data)} className="mt-6 w-full bg-gradient-to-r from-primary to-electric-glow text-primary-foreground">
          <Download className="mr-2 h-4 w-4" />Download PDF Report
        </Button>
      </div>

      <p className="mt-6 text-center text-xs text-muted-foreground">Public verification view · Powered by VoltPass</p>
    </main>
  );
}
