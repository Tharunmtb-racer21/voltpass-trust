...
import { Battery, ShieldCheck, AlertTriangle, Activity } from "lucide-react";
...
const COLORS = ["oklch(0.72 0.19 235)", "oklch(0.72 0.19 150)", "oklch(0.80 0.17 75)", "oklch(0.64 0.22 25)", "oklch(0.65 0.20 290)"];

function Dashboard() {
  const { data: batteries = [] } = useQuery({
    queryKey: ["batteries"],
    queryFn: async () => {
      const { data, error } = await supabase.from("batteries").select("*").order("created_at", { ascending: false });
  ...
  const avg = total ? Math.round(batteries.reduce((a, b) => a + (b.trust_score ?? 0), 0) / total) : 0;
  ...
  const trustBuckets = [
    ...
    { name: "Good (75–89)", value: batteries.filter((b) => (b.trust_score ?? 0) >= 75 && (b.trust_score ?? 0) < 90).length },
    { name: "Moderate (60–74)", value: batteries.filter((b) => (b.trust_score ?? 0) >= 60 && (b.trust_score ?? 0) < 75).length },
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Average Trust Score" value={avg} icon={Activity} tone="primary" suffix="/100" />
        ...
        <Panel title="Trust Score Distribution">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={trustBuckets}>
              ...
              <Tooltip contentStyle={{ background: "oklch(0.21 0.025 250)", border: "1px solid oklch(0.30 0.03 250 / 0.6)", borderRadius: 8 }} />
              <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                {trustBuckets.map((_, i) => <Cell key={i} fill={COLORS[i]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Panel>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={chemData} dataKey="value" nameKey="name" outerRadius={90} label>
                {chemData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
              <Tooltip contentStyle={{ background: "oklch(0.21 0.025 250)", border: "1px solid oklch(0.30 0.03 250 / 0.6)", borderRadius: 8 }} />
              <Legend />
            </PieChart>
        ...
        <Panel title="State of Health vs Trust Score" className="lg:col-span-2">
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={sohData}>
              ...
              <Tooltip contentStyle={{ background: "oklch(0.21 0.025 250)", border: "1px solid oklch(0.30 0.03 250 / 0.6)", borderRadius: 8 }} />
              <Legend />
              <Line type="monotone" dataKey="soh" stroke="oklch(0.72 0.19 150)" strokeWidth={2} dot={{ r: 3 }} name="SOH %" />
              <Line type="monotone" dataKey="score" stroke="oklch(0.72 0.19 235)" strokeWidth={2} dot={{ r: 3 }} name="Trust Score" />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
      </div>
      ...
      <Panel title="Recent Activity" className="mt-6">
        <div className="divide-y divide-border/60">
          {recent.map((b) => {
            ...
            return (
              <Link key={b.id} to="/batteries/$id" params={{ id: b.id }} className="flex items-center justify-between py-3 transition-colors hover:bg-accent/40 -mx-2 px-2 rounded-md">
                <div>
                  <div className="font-medium">{b.battery_code} · <span className="text-muted-foreground">{b.manufacturer} {b.model}</span></div>
                  <div className="text-xs text-muted-foreground">{b.chemistry} · {b.charge_cycles} cycles · {b.current_capacity_kwh}/{b.original_capacity_kwh} kWh</div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs uppercase tracking-wide" style={{ color: toneColor(cat.tone) }}>{cat.label}</span>
                  <span className="font-display text-xl font-bold tabular-nums">{b.trust_score}</span>
                </div>
              </Link>
            );
          })}
...
function StatCard({ label, value, icon: Icon, tone, suffix }: { label: string; value: number; icon: any; tone: string; suffix?: string }) {
  return (
    <div className="glass-card rounded-2xl p-5">
      ...
      <div className="mt-2 font-display text-3xl font-bold tabular-nums">{value}<span className="text-base font-normal text-muted-foreground">{suffix}</span></div>
    </div>
  );
}
