...
import { Shield, Users, Battery, BarChart3, Trash2 } from "lucide-react";
...
function Admin() {
  ...
  const exportCsv = () => {
    const headers = ["battery_code", "manufacturer", "model", "chemistry", "trust_score", "lifecycle_status"];
    const rows = batteries.map((b) => headers.map((h) => JSON.stringify((b as any)[h] ?? "")).join(","));
    const csv = [headers.join(","), ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "voltpass-batteries.csv"; a.click();
  };
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Stat icon={BarChart3} label="Avg Trust" value={batteries.length ? Math.round(batteries.reduce((a, b) => a + (b.trust_score ?? 0), 0) / batteries.length) : 0} />
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-xs uppercase text-muted-foreground">
                <th className="py-2">Code</th><th>Manufacturer</th><th>Chemistry</th><th>Score</th><th>Status</th><th></th>
              </tr>
            </thead>
            <tbody>
              {batteries.map((b) => (
                <tr key={b.id} className="border-b border-border/40">
                  <td className="py-2"><Link to="/batteries/$id" params={{ id: b.id }} className="text-primary hover:underline">{b.battery_code}</Link></td>
                  ...
                  <td><Button size="sm" variant="ghost" onClick={() => remove(b.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-6 glass-card rounded-2xl p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-xs uppercase text-muted-foreground">
                <th className="py-2">Name</th><th>Email</th><th>Organization</th><th>Joined</th>
              </tr>
            </thead>
            <tbody>
              {profiles.map((p) => (
